    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      in
      {
        packages = {
          ...
          default =
            pkgs.runCommand "cmha-all-outputs" # builds outputs to be used by conteiner images only, statically linked
              {
                buildInputs = [
        ...
        devShells.default = pkgs.mkShell {
          nativeBuildInputs = with pkgs.buildPackages; [
            (pkgs.buildPackages.fenix.complete.withComponents [
              "cargo"
              "clippy"
              "rust-src"
              "rustc"
            ...
            yq-go
            gnumake
            buildah # also needed: newgidmap and newuidmap, but nix's don't have setuid bits set for these tools, install them directly
          ];
        };
